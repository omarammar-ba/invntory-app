...
const TileForm:
  React.FC<TileFormProps> = ({
    ...
    const buildSnapshotForSave =
      (valuesToSnapshot: Record<string, string | number> = customValues) => {
        ...
        const currentSnapshots =
          activeCustomFields
            .map(
              field => {
                if (!isCustomFieldActive(field, activeCustomFields, valuesToSnapshot)) {
                  return null;
                }
        ...
        if (!isSystem) {
          ...
          const missingRequiredField =
            activeCustomFields.find(field => {
              ...
              if (!isCustomFieldActive(field, activeCustomFields, cleanCustomValues)) return false;

              if (field.type === 'fixed') {
                return !String(field.fixedValue || '').trim();
              }

              const value = cleanCustomValues[field.id];
          ...
          if (
            ...
            const hasAnyValue =
              activeCustomFields.some(
                field => {
                  if (!isCustomFieldActive(field, activeCustomFields, cleanCustomValues)) {
                    return false;
                  }
          ...
          const tileToSave:
            ...
            customFieldSnapshot:
              buildSnapshotForSave(cleanCustomValues),
        ...
        return (
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {activeCustomFields.map(
                field => {
                  if (!isCustomFieldActive(field, activeCustomFields, customValues)) {
                    return null;
                  }

                  const availableOptions =
                    field.type === 'select'
                      ? getAvailableCustomFieldOptions(field, customValues)
                      : [];
